...
> This skill targets Clerk CLI version `5.9.0`. If `clerk --version` disagrees with the pinned CLI, refresh it with `clerk skill install` or a package runner such as `bunx clerk@5.9.0`. The binary is always the source of truth, so run `clerk <command> --help` to verify anything this skill claims.
...
| ------------------------- | -------------------------------- |
| bun (`bun.lock*`)         | `bunx clerk@5.9.0`     |
| npm (`package-lock.json`) | `npx -y clerk@5.9.0`   |
| pnpm (`pnpm-lock.yaml`)   | `pnpm dlx clerk@5.9.0` |
| yarn >= 2 (`yarn.lock`)   | `yarn dlx clerk@5.9.0` |
...
```sh
# 1. Persist the response in a private temp directory. Use --limit 250 to maximize page size for users list.
workdir="$(mktemp -d -t clerk.XXXXXX)"
trap 'rm -rf "$workdir"' EXIT

clerk users list --json --limit 250 > "$workdir/users.json"
clerk apps list --json               > "$workdir/apps.json"
clerk api /users/user_abc123         > "$workdir/user.json"

# 2. Inspect only what you need.
jq '.data | length'                       "$workdir/users.json"   # current page size
jq '.hasMore'                             "$workdir/users.json"   # are more pages available?
jq '.data[0] | keys'                      "$workdir/users.json"   # discover the user shape once
jq '.data[] | {id, primary_email_address_id}' "$workdir/users.json"   # project to the minimum needed fields
jq '[.data[] | select(.banned)] | length' "$workdir/users.json"   # aggregate without reading rows
```

**If `jq` is not available**, fall back to Python or Node - both can stream the file without printing it whole:

```sh
python3 -c 'import json; d=json.load(open("$workdir/users.json")); print(len(d["data"]), d["hasMore"])'
node -e 'const d=require(process.env.WORKDIR + "/users.json"); console.log(d.data.length, d.hasMore)'
```

`cat` / `head` the file only when you genuinely need to see the raw structure for one-off debugging. When walking pages, write each page to its own file under the temp directory (e.g. `page-${offset}.json`) so individual pages stay independently inspectable.
