...
compatibility: Requires a framework-appropriate publishable key such as NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY for Next.js, VITE_CLERK_PUBLISHABLE_KEY for Vite, EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY for Expo, or the equivalent publishable key for your framework. CLERK_SECRET_KEY is required only for server-side or dashboard flows; it is not a universal client-side prerequisite. Keys can be auto-generated via Keyless on first SDK initialization, or pulled from the Clerk Dashboard. Requires Node.js 20.9.0 or higher.
metadata:
  author: clerk
  version: 2.3.0
---
...
```bash
# Ask the user for explicit confirmation before running this step.
clerk init --framework <next|react|vue|nuxt|astro|react-router|tanstack-react-start|expressjs|fastify|expo> -y
```

Do not run `clerk init -y` automatically. Ask the user for explicit confirmation first, because it provisions the Clerk app, links the project, writes framework-specific publishable + secret keys to the env file (e.g. `.env.local` for Next.js, `.env` for Vite-based projects), and installs the SDK package.
...
```bash
clerk api --platform /v1/platform/applications/<app_id>/rotate_secret_keys -X POST \
