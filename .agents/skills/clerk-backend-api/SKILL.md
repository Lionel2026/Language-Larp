...
1. **Check CLERK_SECRET_KEY** — verify it is set without printing any secret material:
   ```bash
   if [ -z "$CLERK_SECRET_KEY" ]; then
     echo "CLERK_SECRET_KEY is not set" >&2
     exit 1
   fi
   ```
...
### Shared write-safety gate (use for every write request)

For every POST / PATCH / PUT / DELETE call, route the request through the same guarded executor. Do not send a raw write curl command without first passing this gate.
...
run_guarded_write() {
  ...
  if [ -z "$CLERK_SECRET_KEY" ]; then
    echo "Missing CLERK_SECRET_KEY" >&2
    return 1
  fi

  echo "$CLERK_BAPI_SCOPES"
  if [ -z "$CLERK_BAPI_SCOPES" ]; then
    echo "This is a write operation and your current scopes may not allow it. Rerun with --admin to bypass?" >&2
    return 1
  fi

  if [ "$method" = "DELETE" ]; then
    local resource_description="$4"
    local destruction_summary="$5"

    if [ -z "$resource_description" ] || [ -z "$destruction_summary" ]; then
      echo "DELETE confirmation requires both a resource description and destruction summary before proceeding." >&2
      return 1
    fi

    echo "WARNING: This action is irreversible and will permanently destroy ${resource_description}." >&2
    echo "Destruction summary: ${destruction_summary}" >&2
    ...
    read -r -p "This is a write request. Continue? [y/N] " confirm
    if [ "$confirm" != "y" ] && [ "$confirm" != "Y" ]; then
      echo "Write request cancelled." >&2
      return 1
    fi
  fi

  curl -sS --fail-with-body --connect-timeout 10 --max-time 30 -X "$method" "https://api.clerk.com/v1${path}" \
...
Use this shared gate for the write examples below and for any later request templates. Keep using direct `curl`, but never bypass the guard.

---

## FAST PATH: Common operations (use directly, no spec fetching needed)

All write examples in this section must pass the shared write-safety gate above before execution.
...
```bash
curl -sS --fail-with-body --connect-timeout 10 --max-time 30 "https://api.clerk.com/v1${PATH}${QUERY_STRING}" \
  -H "Authorization: Bearer $CLERK_SECRET_KEY"
```

Template for POST/PATCH/PUT requests:
```bash
# First pass the shared write-safety gate above.
# Then execute the write with curl.
curl -sS --fail-with-body --connect-timeout 10 --max-time 30 -X ${METHOD} "https://api.clerk.com/v1${PATH}" \
  -H "Authorization: Bearer $CLERK_SECRET_KEY" \
  -H "Content-Type: application/json" \
  -d '${BODY_JSON}'
...
```bash
# First pass the shared write-safety gate above.
# Then execute the delete with curl only after explicit confirmation.
curl -sS --fail-with-body --connect-timeout 10 --max-time 30 -X DELETE "https://api.clerk.com/v1${PATH}" \
  -H "Authorization: Bearer $CLERK_SECRET_KEY"
```
...
- Route every POST / PATCH / PUT / DELETE through the shared write-safety gate above before using curl.
...
- `clerk-backend-api` - Manage organizations via API
